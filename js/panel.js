(function () {
  ...
  var lastSnapshot,
    ...
    xpathText = $('#element-xpath'),
    feedbackElement = xpathText.parent(),
    xpathForm = $('#xpath-form'),
  ...
  xpathForm.on('submit', function (event) {
    event.preventDefault();

    feedbackElement.removeClass('has-error').removeClass('has-success');

    // Ask the content script to take a snapshot of the element we are referencing
  ...
  function saveAndProcessSnapshot(snapshooterResult) {
    lastSnapshot = JSON.parse(snapshooterResult);
    processSnapshot();
  }
  ...
  chrome.runtime.onMessage.addListener(function (request, sender, cb) {
    var snapshooterResult;
    // Note: Setting textarea's value and text node's nodeValue is XSS-safe.
    if (request.type === 'evaluated') {
      snapshooterResult = request.result;
      if (!snapshooterResult) {
        feedbackElement.addClass('has-error');
        return;
      }
      feedbackElement.addClass('has-success');
      saveAndProcessSnapshot(snapshooterResult);
    }
  });
})();
